...
describe('[F13] Security Module', () => {
  ...
  const TEST_FEED = 'https://blog.example.com/feed';
  ...
  it('[F13-03] should generate HMAC signature with static nonce', () => {
    const sig1 = generateSignature(TEST_USERNAME, TEST_SECRET, TEST_FEED);
    const sig2 = generateSignature(TEST_USERNAME, TEST_SECRET, TEST_FEED);
  ...
  it('[F13-04] should verify valid signature correctly', () => {
    const signature = generateSignature(TEST_USERNAME, TEST_SECRET, TEST_FEED);
    const isValid = verifySignature(TEST_USERNAME, signature, TEST_SECRET, TEST_FEED);
  ...
  it('[F13-04] should reject invalid signature', () => {
    ...
    const isValid = verifySignature(TEST_USERNAME, invalidSignature, TEST_SECRET, TEST_FEED);
  ...
  it('[F13-04] should reject tampered username', () => {
    const signature = generateSignature(TEST_USERNAME, TEST_SECRET, TEST_FEED);
    const isValid = verifySignature('different-user', signature, TEST_SECRET, TEST_FEED);
  ...
  it('[F13-05] should integrate validation in request handler', () => {
    ...
    const validResult = validateRequest(TEST_USERNAME, signature, TEST_SECRET, true, TEST_FEED);
    const invalidResult = validateRequest(TEST_USERNAME, 'wrong-sig', TEST_SECRET, true, TEST_FEED);
  ...
  it('[F13-06] should cache failed attempts', () => {
    ...
    for (let i = 0; i < 12; i++) {
      validateRequest(TEST_USERNAME, badSig, TEST_SECRET, true, TEST_FEED);
    }
    
    // Next attempt should be cached
    const result = validateRequest(TEST_USERNAME, badSig, TEST_SECRET, true, TEST_FEED);
